...
parser.add_argument('--lr', type=float, default=0.000125) #yuan 0.001
...
for ep in tqdm(range(args.epochs), desc="Training"):
    ...
    for batch_idx, (xx, yy, msk) in enumerate(tqdm(train_loader, desc=f"Epoch {ep+1}/{args.epochs}")):
        ...
        if batch_idx % args.grad_accum == 0:
            optimizer.zero_grad()
        total_loss = loss / max(1, args.grad_accum)  # + 1.0 * cls_loss
        total_loss.backward()
        
        flush_step = ((batch_idx + 1) % args.grad_accum == 0) or ((batch_idx + 1) == len(train_loader))
        if flush_step:
            nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)
            optimizer.step()
            scheduler.step()
            
            # For AdaLoRA, update importance scores and rank allocation (on optimizer steps)
            if args.peft_method == 'adalora':
                global_step = ep * (len(train_loader) // max(1, args.grad_accum)) + ((batch_idx + 1) // max(1, args.grad_accum))
                if ((batch_idx + 1) // max(1, args.grad_accum)) % max(1, args.adalora_update_freq) == 0:
