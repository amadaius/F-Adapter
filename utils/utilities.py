    ...
    if (components == 'all') or ('all' in components):
        ...
        for name in components:
            if name == 'patch_embed' and hasattr(model, 'patch_embed'):
                ...
                print(f"Submodule does not exist: {name}")
        return



def load_3d_components_from_2d(model, state_dict, components='all', strict=False, exclude_peft_keywords=None):
    """
    :model: the model
    :state_dict: state_dict of source model
    :components: 'all' or list from 'patch', 'pos', 'blocks','time_agg','cls_head', 'scale_feats', 'out'
    :strict: bool (default False). If True, will enforce that the keys in `state_dict` match exactly 
             the keys expected by the model. If False, missing keys or unexpected keys will be ignored.
    :exclude_peft_keywords: list of keywords to exclude (e.g., ['lora', 'adapter'])
    """

    # Default PEFT keywords to exclude
    if exclude_peft_keywords is None:
        exclude_peft_keywords = []
    
    def should_exclude_key(key):
        """Check whether a key should be excluded (PEFT-related)"""
        key_lower = key.lower()
        return any(keyword in key_lower for keyword in exclude_peft_keywords)
    
    if next(iter(state_dict.keys())).startswith('module.'):
        # If checkpoint was saved with DataParallel, strip the 'module.' prefix
        new_state_dict = OrderedDict()
        for key, item in state_dict.items():
            new_key = key.replace('module.', '')
            new_state_dict[new_key] = item
    ...
    if (components == 'all') or ('all' in components):
        ...
        for name in components:
            if name == 'blocks' and hasattr(model, 'blocks'):
                for i, block in enumerate(model.blocks):
                    # For each block, pull its keys from state_dict (excluding PEFT-related)
                    block_state_dict = OrderedDict({
                        k.replace(f'blocks.{i}.', ''): v
                        for k, v in state_dict.items()
                        if k.startswith(f'blocks.{i}.') and not should_exclude_key(k)
                    })
                    
                    # Reshape Conv2d weights to Conv3d weights (only when weights are 4D)
                    for k, v in block_state_dict.items():
                        if 'mlp' in k and 'weight' in k and v.ndim == 4:  # Conv2d(4D) -> Conv3d(5D)
                            block_state_dict[k] = v.unsqueeze(-1)  # [out,in,h,w] -> [out,in,d,h,w]
                        elif 'mlp' in k and 'weight' in k and v.ndim == 5:
                            pass  # Already Conv3d weights, no change
                        elif 'mlp' in k and 'weight' in k:
                            raise RuntimeError(f"Unexpected weight dimension {v.ndim} for {k}, expected 4D or 5D")

                    # Load parameters for this block
                    try:
                        missing_keys, unexpected_keys = block.load_state_dict(block_state_dict, strict=False)
                        
                        # Check for missing critical weights
                        critical_missing = [k for k in missing_keys if 'mlp' in k and 'weight' in k]
                        if critical_missing and not exclude_peft_keywords:
                            print(f"Block {i} critical weights missing: {critical_missing}")
                        elif exclude_peft_keywords and missing_keys:
                            # Filter out PEFT-related missing keys
                            non_peft_missing = [k for k in missing_keys 
                                                  if not any(keyword in k.lower() for keyword in exclude_peft_keywords)]
                            if non_peft_missing:
                                print(f"Warning: Block {i} missing non-PEFT weights: {non_peft_missing[:3]}...")
                                
                    except RuntimeError as e:
                        if exclude_peft_keywords:
                            print(f"Block {i} failed to load: {str(e)}")
                        else:
                            raise e

            elif name == 'time_agg' and hasattr(model, 'time_agg_layer'):
                # Load only the time_agg_layer submodule
                time_agg_dict = OrderedDict({
                    k.replace('time_agg_layer.', ''): v
                    for k, v in state_dict.items()
                    if k.startswith('time_agg_layer.')
                })
                model.time_agg_layer.load_state_dict(time_agg_dict, strict=strict)

            # Additional elif blocks can be added for 'patch', 'pos', 'cls_head', etc.
            else:
                print(f"Submodule does not exist or is not handled: {name}")
        return
